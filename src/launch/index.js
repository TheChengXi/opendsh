/**
 * @intent
 * 五模式启动分发器：把五启动模式（integrated/window/hidden/window-keepalive/hidden-keepalive）聚为一族，
 ...
 *
 * 验收条件：
 * - createLauncher(deps) 构造五模式启动器并注册到 mode→launcher 映射
...
function createLauncher(deps) {
  const launchers = {
    ...
    hidden: hidden.createHiddenLauncher(deps),
