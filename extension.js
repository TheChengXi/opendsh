/**
 * @intent
 * VS Code 激活入口：注册 open/start/stop 命令与 URI 深链，读设置并交予 manager，自身无业务逻辑。
 *
 * 边界：open 优先 simpleBrowser.api.open、失败回退 openExternal；URI 仅处理 /open 路径。
 *
 * 验收条件：
 * - 注册 opendsh.open / opendsh.start / opendsh.stop 三条命令
...
function activate(context) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('opendsh.start', manager.start));
